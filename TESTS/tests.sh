...
echo "Tests de la comparaison d'image"
...
do
   imagefile=${testfile%.test}.png
   assert "./raytrace.sh $testfile" ""
   assert "./compare.sh $imagefile ${imagefile#TEST7/}" "OK\n0\n"
done
...
read -p "Voulez vous suprimer les images ? (y/n) " var
[ $var != "y" ] && exit 0
for img in `ls *.png`
do
    rm $img
